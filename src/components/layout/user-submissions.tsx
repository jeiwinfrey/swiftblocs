...
export function UserSubmissions() {
    ...
    if (!authLoading && user) {
      const fetchUserComponents = async () => {
        try {
          ...
          // Filter components by the current user
          const userComponents = allComponents.filter(
            component => component.author === userProfile.username
          );
      ...
      // If auth is done loading but no user, set loading to false
      setIsLoading(false);
    }
  ...
  if (!user) {
    return <div className="p-4">Please sign in to view your submissions.</div>;
  }
